...
def static_pa_script(hp, pool):
    ...
    plt.subplot(341)
    plt.title('Latency per Trial, {:.1f}'.format(np.mean(np.mean(totlat, axis=0)[-16:-6])))
    ...
    for t in range(trials):
        ...
        if hp['platform'] == 'laptop' or b == 0:
            ...
            print('T {} | S {} | D {:4.3f} | {}as | g{} | Dgr {}'.format(
                ...
                np.round(np.mean(agent.actor.avgspeed), 3), np.round(goal,2), np.round(dgr)))
...
if __name__ == '__main__':
    hp = get_default_hp(task='center', platform='server')
    ...
    hp['alr'] = 0.00001  # 0.00001
    hp['clr'] = 0.0001  # 0.0001
    hp['taug'] = 10000

    hp['usenmc'] = True  # confi, neural
    hp['contbeta'] = 0.3  # 1 for motor controller, 0 for actor
    ...
    for cb in allcb:
        ...
        hp['exptname'] = 'center_res_{}cb_{}ach_{}glr_{}sl_{}clr_{}tg_{}alr_{}R_{}dt_b{}_{}'.format(
