...
def npapa_script(hp, pool):
    ...
    for pt in range(12):
        ...
        plt.scatter(np.array(k[pt])[0, 0], np.array(k[pt])[0, 1], color=col[pt], alpha=0.5, linewidth=1, zorder=1,
                    marker='s', s=50)
        plt.scatter(np.array(k[pt])[-2, 0], np.array(k[pt])[-2, 1], color=col[pt], alpha=0.5, linewidth=1, zorder=2,
                    marker='x', s=50)
...
def sym_12pa_expt(hp, b):
    from backend.model import FFACAgent
    ...
    start = dt.time()
    agent = FFACAgent(hp=hp)

    # Start Training
    _, trw, dgr[:3], pi[:3] = run_sym_12pa_expt(b, 'train', env, hp, agent, alldyn, trsess, noreward=nonrp)

    mvpath, npa1w, dgr[3], pi[3] = run_sym_12pa_expt(b, '12npa', env, hp, agent, alldyn, evsess, trw,
                                                     noreward=[nonrp[0]])
...
def run_sym_12pa_expt(b, mtype, env, hp, agent, alldyn, sessions, useweight=None, nocue=None, noreward=None):
    ...
    for t in range(sessions * env.totr):
        ...
        while not done:
            ...
            if done:
                if hp['gdecay'] == 'done':
                    goal = agent.forget_goal_rfr(state=state, cue=cue, reward=reward, done=done, plasticity=plasticity)
                    # agent.target.reset_goal(reward=reward, done=done, plasticity=plasticity)
                env.render()
                break
        ...
        if hp['platform'] == 'laptop' or b == 0:
            ...
            print('T {} | C {} | S {} | Dgr {} | gl {} | goal {} | '.format(
                t, np.argmax(env.cue) + 1, env.i // (1000 // env.tstep), np.round(env.dgr, 1), env.rloc,
                np.round(goal, 2)))
    ...
    for N in allN:
        ...
        if N > 1200:
            for lr in [7.5e-6, 5e-6, 2.5e-6, 1e-6]:
                ...
                hp['exptname'] = '12pa_ff_{}cb_{}ach_{}mb_{}glr_{}sl_{}clr_{}tg_{}alr_{}R_{}dt_b{}_{}'.format(
                    hp['contbeta'], hp['ach'], hp['mcbeta'], hp['glr'], hp['stochlearn'], hp['clr'], hp['taug'],
            ...
            hp['exptname'] = '12pa_ff_{}cb_{}ach_{}mb_{}glr_{}sl_{}clr_{}tg_{}alr_{}R_{}dt_b{}_{}'.format(
                hp['contbeta'], hp['ach'], hp['mcbeta'], hp['glr'], hp['stochlearn'], hp['clr'], hp['taug'], hp['alr'],
