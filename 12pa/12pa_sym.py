...
def npapa_script(hp, pool):
    ...
    for b in range(btstp):
        totdgr[b], totpi[b], totpath[b], memory, allw = x[b]
    ...
    plt.subplot(234)
    plt.imshow(memory[0],aspect='auto')
    plt.title('OPA Memory')
    plt.colorbar()

    plt.subplot(235)
    plt.imshow(memory[1],aspect='auto')
    plt.title('NPA Memory')
    plt.colorbar()
    ...
    for pt in range(12):
        ...
        plt.scatter(np.array(k[pt])[0, 0], np.array(k[pt])[0, 1], color=col[pt], alpha=0.5, linewidth=1, zorder=1, marker='s',s=10)
        plt.scatter(np.array(k[pt])[-2, 0], np.array(k[pt])[-2, 1], color=col[pt], alpha=0.5, linewidth=1, zorder=2, marker='x', s=10)
    ...
    if hp['savegenvar']:
        saveload('save', './Data/genvars_{:.2g}n_{}'.format(
            np.mean(totpi,axis=0)[3],exptname), [totdgr, totpi, totpath])

    return totdgr, totpi, totpath, memory, allw
...
def sym_12pa_expt(hp,b):
    ...
    if hp['savevar']:
        saveload('save', 'vars_{}_{}'.format(exptname, dt.time()), [mvpath, dgr, pi, [memt,mem1]])
...
def run_sym_12pa_expt(b, mtype, env, hp, agent, alldyn, sessions, useweight=None, nocue=None, noreward=None):
    ...
    for t in range(sessions*env.totr):
        ...
        if hp['platform'] == 'laptop' or b == 0:
            # Trial information
            print('T {} | C {} | S {} | d {} | Dgr {:.2g} | gl {} | goal {} | '.format(
                t, np.argmax(env.cue)+1, env.i // (1000 // env.tstep), np.round(ds4r,2), np.round(env.dgr,1), env.rloc, np.round(goal,2)))
    ...
    return mvpath, mdlw, dgr, sesspi, np.copy(agent.memory.keyvalue)
...
if __name__ == '__main__':
    ...
    for cb in allcb:
        ...
        hp['exptname'] = '12pa_sym_{}cb_{}mb_{}oe_{}gd{}_{}clr_{}tg_{}alr_{}R_{}dt_b{}_{}'.format(
            hp['contbeta'], hp['mcbeta'], hp['omite'], hp['gdecay'],hp['gdist'], hp['clr'], hp['taug'], hp['alr'],
            hp['Rval'], hp['tstep'], hp['btstp'], dt.monotonic())

        totdgr, totpi, totpath, memory, allw = npapa_script(hp, pool)
